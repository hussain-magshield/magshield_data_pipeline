...
def build_lookups():
    ...
    orgs = {str(o["ORGANISATION_ID"]): o.get("ORGANISATION_NAME", "") for o in results["orgs"]}
    users = {str(u["USER_ID"]): f'{u.get("USER_ID")};{u.get("FIRST_NAME","")} {u.get("LAST_NAME","")}' for u in results["users"]}
    pricebooks = {str(p["PRICEBOOK_ID"]): p.get("NAME", "") for p in results["pricebook"]}
    products = {str(p["PRODUCT_ID"]): p.get("PRODUCT_FAMILY", "") for p in results["product"]}
    state_reason_map = {str(r["STATE_REASON_ID"]): r.get("STATE_REASON", "") for r in results["state_reason"]}
    return orgs, users, pricebooks, products, state_reason_map
...
def get_product_id_from_pricebook_entry(entry_id):
    ...
    if r and r.status_code == 200:
        ...
        pricebook_entry_cache[entry_id] = pid
        return pid
    return None

# ==============================
#  Build Opportunity Product Map
# ==============================
def build_opp_product_map():
    opp_product_map = {}
    line_items = fetch_all_paged("OpportunityLineItem")
    with ThreadPoolExecutor(max_workers=10) as ex:
        futures = {ex.submit(get_product_id_from_pricebook_entry, li.get("PRICEBOOK_ENTRY_ID")): li for li in line_items}
        for f in as_completed(futures):
            li = futures[f]
            prod_id = f.result()
            if prod_id:
                opp_id = str(li.get("OPPORTUNITY_ID"))
                opp_product_map.setdefault(opp_id, []).append(str(prod_id))
    return opp_product_map
...
def fetch_stage_name(stage_id):
    ...
    if stage_id in stage_cache: return stage_cache[stage_id]
    r = safe_get(f"{BASE_URL}/PipelineStages/{stage_id}")
    if r and r.status_code == 200:
        name = r.json().get("STAGE_NAME", "")
...
def fetch_site_name(opportunity_id, main_org_id, orgs):
    ...
    site_names = [
        ...
        for l in links
        if l.get("LINK_OBJECT_NAME") == "Organisation" and str(l.get("LINK_OBJECT_ID")) != str(main_org_id)
    ]
...
def main_opportunity():
    ...
    stage_name_map = {}
    with ThreadPoolExecutor(max_workers=10) as ex:
        site_futures = {ex.submit(fetch_site_name, opp.get("OPPORTUNITY_ID"), opp.get("ORGANISATION_ID"), organisations): opp for opp in opportunities}
        stage_futures = {ex.submit(fetch_stage_name, opp.get("STAGE_ID")): opp for opp in opportunities}
        ...
        for f in as_completed(stage_futures):
            opp = stage_futures[f]
            stage_name_map[str(opp.get("STAGE_ID"))] = f.result() or ""
    ...
    if rows:
        ...
        logging.info(f"Exported {len(rows)} opportunities to {output_file}")
        return output_file
    else:
